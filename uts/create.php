...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($input_nama)){
        $nama_err = "Please Enter A Valid Nama Pembeli!";
    ...
    if (empty($input_harga)){
        @$harga_err = "Please Choose the Product!";
    } else{
        $harga = $input_harga;
    }
    ...
    if (empty($nama_err) && empty($tgl_err) && empty($product_err) && empty($harga_err)) {
        $sql = "INSERT INTO produk (nama, tgl, product, harga) VALUES (?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($con, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssi", $param_nama, $param_tgl, $param_product, $param_harga);
            $param_nama = $nama;
            $param_tgl = $tgl;
